...
contract PermissionlessPaymaster is IPaymaster, EIP712 {
    ...
    /// @notice Rescue address managed by Zyfi to rescue mistakenly sent ERC20 tokens.
    ...
    */
        ...
        returns (bytes4 magic, bytes memory context)
    {
        ...
        if (paymasterInputSelector == IPaymasterFlow.general.selector) {
            ...
            */
            ...
            if (_balance < requiredETH) revert Errors.PM_InsufficientBalance();
    ...
    function replaceSigner(address _oldSigner, address _newSigner) public {
        ...
        // Old signer should not be registered
        if (managers[_newSigner] != address(0))
            revert Errors.PM_SignerAlreadyRegistered();
        // New signer should be registered with manager
     ...
     * @dev Only ZYFI_RESCUE_ADDRESS can call this method
     */
    function updateRescueAddress(address _newAddress) public {
        if (msg.sender != ZYFI_RESCUE_ADDRESS) revert Errors.PM_Unauthorized();
     ...
     */
    function getManagerBalance_via_Signer(
        address _signer
    ) public view returns (uint balance) {
        balance = managerBalances[managers[_signer]];
    }
}