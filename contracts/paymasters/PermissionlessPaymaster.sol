...
contract PermissionlessPaymaster is IPaymaster, EIP712 {
    ...
    /// @notice Address managed by Zyfi Dao to rescue mistakenly sent ERC20 tokens, to set markup percent & collect markup fee.
    address public zyfi_dao_manager;
    ...
    mapping(address manager => uint256 ethBalance) public managerBalances;
    ...
    uint256 public previousTotalBalance;

    /// @notice Markup percent to be charged on every transaction. Can only be set by Zyfi Dao Manager
    uint256 public markupPercent;
    ...
    event Deposit(address indexed manager, uint256 amount);
    ...
    /// @notice Event to be emitted when zyfi_dao_manager is changed
    event DAO_ManagerChanged(
        address indexed oldAddress,
        address indexed newAddress
    );
    /// @notice Event to be emitted when markup is changed
    event NewMarkupPercent(uint256 oldMarkup, uint256 newMarkup);
    ...
    */
    function updateRefund(uint256 amount, bool isWithdraw) internal {
        uint256 _previousTotalBalance = previousTotalBalance;
        ...
        returns (bytes4 magic, bytes memory context)
    {
        ...
        if (paymasterInputSelector == IPaymasterFlow.general.selector) {
            ...
            */
            ...
            ) = abi.decode(innerInputs, (uint256, uint256, address, bytes));
            ...
            if (markupPercent > 0) {
                ...
                // Add the markup to the zyfi_dao_manager balance
                managerBalances[zyfi_dao_manager] += markup;
            ...
            uint256 _balance = managerBalances[_manager];
     ...
     * @dev Only zyfi_dao_manager can call this method
     */
    function updateDaoManager(address _newAddress) public {
        address _oldDaoManager = zyfi_dao_manager;
        if (msg.sender != _oldDaoManager) revert Errors.PM_Unauthorized();
        if (_newAddress == address(0)) revert Errors.PM_InvalidAddress();
        updateRefund(0, false);
        // Transfer the balance to the new address
        uint256 balance = managerBalances[_oldDaoManager];
        managerBalances[_oldDaoManager] = 0;
        managerBalances[_newAddress] += balance;
        zyfi_dao_manager = _newAddress;
        emit DAO_ManagerChanged(_oldDaoManager, _newAddress);
    }
    function updateMarkupPercent(uint256 _newMarkupPercent) public{
        if(msg.sender != zyfi_dao_manager) revert Errors.PM_Unauthorized();
        if(_newMarkupPercent > DENOMINATION) revert Errors.PM_Unauthorized();
        uint256 oldMarkup = markupPercent;
        markupPercent = _newMarkupPercent;
        emit NewMarkupPercent(oldMarkup, _newMarkupPercent);
    }
    /**
